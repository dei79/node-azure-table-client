...
function AzureModel(definition, definitionObject, defaults) {
    ...
    self.TableName      = definition.TableName;
    ...
    /*
     * Returns a list of data keys defined in the model
     */
    self.modelKeys = function() {
        return definitionObject.modelKeys();
    }
}
...
function AzureModelDefinition(definition) {
    ...
    self.query = function(partitionKey, rowKey) {
        ...
        return self.queryNative(query)
    };

    self.queryNative = function(query) {
        var deferred = q.defer();
...
/*
 * Allows to configure the backend which is used
 * for all storage operations
 */
exports.setBackend = function(backendDriver) {
